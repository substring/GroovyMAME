...
/***************************************************************************
    ...
    The core is loaded from -libretro_core and runs one retro_run per
    emulated frame, so GroovyMAME's switchres, frame delay and vsync
    handling apply to it like to any other system.
    ...
    Usage:
      groovymame libretro -libretro_core genesis_plus_gx_libretro.so
                          -libretro_content sonic.md

    Directories:
      -libretro_system_path   system directory given to the core (BIOS)
...
void libretro_state::load_content()
{
	...
	if (m_content_path.empty())
	{
		if (!m_support_no_game)
			throw emu_fatalerror("libretro: %s needs content, use -libretro_content\n", m_library_name);
	...
	game.path = m_content_path.c_str();

	// cores that don't need a path get the content in memory
	if (!info.need_fullpath)
	{
		std::ifstream file(m_content_path, std::ios::binary);
		if (!file)
...
void libretro_state::machine_start()
{
	...
	m_content_path = options.libretro_content();
	m_system_dir = options.libretro_system_path();
	m_save_dir = std::string(options.nvram_directory()) + PATH_SEPARATOR "libretro";

	if (m_core_path.empty())
		throw emu_fatalerror("libretro: no core specified, use -libretro_core\n");

	size_t const slash = m_content_path.find_last_of("/\\");
	m_content_name = (slash == std::string::npos) ? m_content_path : m_content_path.substr(slash + 1);
